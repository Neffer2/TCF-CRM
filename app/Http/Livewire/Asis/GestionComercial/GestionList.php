...
class GestionList extends Component
{
    ...
    { 
        ...
        $datos = GestionComercial::select('id','id_contacto','id_estado', 'nom_proyecto_cot')->where('id_user', $asistente->comercial_id)->orWhereHas('participantes', function ($p) use ($asistente) { $p->where('user_id', $asistente->comercial_id); })->orderBy('id', 'desc')->paginate(10);
